...
class Chunk:
    ...
    def write_constant(self, idx: int, line: int = 0):
        self.code.append(idx & 0xFF)
        self.lines.append(line)
    ...
    def patch_jump(self, offset: int):
        ...
        self.code[offset] = (jump_target - offset - 2) & 0xFF
        self.code[offset + 1] = ((jump_target - offset - 2) >> 8) & 0xFF
    
    def emit_loop(self, loop_start: int, line: int = 0) -> int:
        self.write(OpCode.LOOP, line)
        offset = len(self.code) - loop_start + 3
        ...
        while i < len(self.code):
            ...
            if opcode in (OpCode.CONSTANT, OpCode.GET_GLOBAL, OpCode.SET_GLOBAL,
                ...
                if i + 2 <= len(self.code):
                    offset = self.code[i + 1] | (self.code[i + 2] << 8)
                    target = i + 3 + offset
