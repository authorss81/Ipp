...
class Compiler:
    ...
    def compile_function(self, node: FunctionDecl):
        ...
        self.chunk.add_constant(func_chunk, self.current_line)
    ...
    def compile_class(self, node: ClassDecl):
        ...
        if node.superclass:
            self.compile_expr(node.superclass)
        ...
        for method in node.methods:
            if isinstance(method, FunctionDecl):
                ...
                self.chunk.add_constant(method.name, self.current_line)
    ...
    def compile_import(self, node: ImportDecl):
        ...
        self.chunk.add_constant(node.module_path, self.current_line)
        
        if node.alias:
            self.chunk.add_constant(node.alias, self.current_line)
        elif node.imports:
            for imp_name in node.imports:
                self.chunk.add_constant(imp_name, self.current_line)
    ...
    def compile_identifier(self, name: str):
        ...
        if idx is not None:
            ...
            self.chunk.add_constant(name, self.current_line)
    ...
    def compile_optional_chain(self, node: OptionalChainingExpr):
        ...
        for access in node.accesses:
            if isinstance(access, str):
                ...
                self.chunk.add_constant(access, self.current_line)
            else:
